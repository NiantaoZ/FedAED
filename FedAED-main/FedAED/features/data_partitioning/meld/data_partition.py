...
def data_partition(
    ...
    for i, df_row in tqdm(df_label.iterrows()):
        if not Path(f"{data_path}/waves/dia{df_row.Dialogue_ID}_utt{df_row.Utterance_ID}.wav").is_file():  #wav
            err.append(i)
    ...
    df_label_cleaned['Path'] = df_label_cleaned.apply(lambda row: f"{data_path}/waves/dia{row.Dialogue_ID}_utt{row.Utterance_ID}.wav", axis=1) #wav
