...
class FeatureManager():
    ...
    def extract_mfcc_features(
        ...
        # 确保在运行时可以找到ffmpeg的可执行文件
        # ffmpeg_path = r"E:\ffmpeg-7.0.2-essentials_build\bin"  # 这里替换为你系统上ffmpeg的路径
        # os.environ["PATH"] += os.pathsep + ffmpeg_path

        audio_path = Path(audio_path)
        #
        # # 如果是 MP4 格式，先转换为 WAV 格式
        # if audio_path.suffix.lower() == '.mp4':
        #     wav_path = audio_path.with_suffix('.wav')
        #
        #     # 使用 ffmpeg-python 转换 MP4 为 WAV
        #     ffmpeg.input(str(audio_path)).output(str(wav_path), acodec='pcm_s16le', ar='16000', ac=1).run()
        #
        #     audio_path = wav_path  # 修改为转换后的 WAV 文件路径
        #     ##
