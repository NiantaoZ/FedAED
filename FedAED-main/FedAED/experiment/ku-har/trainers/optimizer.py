    ...
    def step(self, server_control, client_control, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                # print(f"Gradient shape: {d_p.shape}")
                # print(f"Server control shape: {c.shape}")
                # print(f"Client control shape: {ci.shape}")
...
class pFedMeOptimizer(torch.optim.Optimizer):
    def __init__(self, params, lr=0.01, lamda=0.1, mu=0.001):
        if lr < 0.0:
            raise ValueError("Invalid learning rate: {}".format(lr))
        if lamda < 0.0:
            raise ValueError("Invalid lamda value: {}".format(lamda))
        if mu < 0.0:
            raise ValueError("Invalid mu value: {}".format(mu))
            
        defaults = dict(lr=0.01, lamda=0.5, mu=0.001)
        super(pFedMeOptimizer, self).__init__(params, defaults)

    def step(self, local_model, device=None, closure=None):
        """Performs a single optimization step.
        Args:
            local_model: The local model parameters to approximate
            device: The device to move tensors to (optional)
            closure (callable, optional): A closure that reevaluates the model
                and returns the loss.
        """
        loss = None
        ...
        # Make sure local_model is a list of parameters
        if not isinstance(local_model, list):
            local_model = list(local_model.parameters())
            
        for group in self.param_groups:
            for p, local_p in zip(group['params'], local_model):
                if p.grad is None:
                    continue
                
                # Move local model parameter to the same device as p
                ...
                # Add the proximal term (difference from local model)
                if group['lamda'] != 0:
                    d_p.add_(group['lamda'], p.data - local_p.data)
                
                # Add the regularization term
                if group['mu'] != 0:
                    d_p.add_(group['mu'], p.data)
                
                # Update parameters
                p.data.add_(-group['lr'], d_p)
                
        return group['params']  # 保持与原始实现相同的返回类型
        
class PerAvgOptimizer(torch.optim.Optimizer):
    r"""Implements Personalized Averaging (PerAvg) algorithm.
    PerAvg performs two-step optimization with different learning rates.
    ...
    Example:
        >>> optimizer = PerAvgOptimizer(model.parameters(), lr=0.1)
        >>> optimizer.zero_grad()
        >>> loss_fn(model(input), target).backward()
        >>> optimizer.step(beta=0.01)  # second step with personalized learning rate
    """
    ...
    def __init__(
        ...
        # Track if we're in the first step (using lr) or second step (using beta)
        self._step_count = 0

    def __setstate__(self, state):
        super(PerAvgOptimizer, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('nesterov', False)

    def step(self, beta=0, closure=None):
        """Performs a single optimization step.
        
        Args:
            beta (float): personalized learning rate for second step. 
                         If 0, uses the base learning rate (first step).
            closure (callable, optional): A closure that reevaluates the model
                and returns the loss.
        """
        loss = None
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                # Determine learning rate (base lr or personalized beta)
