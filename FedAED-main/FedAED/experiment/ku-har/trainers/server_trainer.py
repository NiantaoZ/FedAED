...
#from fed_multimodal.trainers.fed_FAED_trainer import ClientFAED
...
class Server(object):
    def __init__(
        ...
        if self.args.fed_alg == 'scaffold':
            ...
            # # 查看服务器控制变量的形状
            # print("Server Control Shapes:")
            # for name, tensor in self.server_control.items():
            #     print(f"  {name}: {tensor.shape}")
            #
            # # 查看每个客户端控制变量的形状
            # print("\nClient Controls Shapes:")
            # for client_id, controls in self.client_controls.items():
            #     print(f"Client {client_id}:")
            #     for name, tensor in controls.items():
            #         print(f"  {name}: {tensor.shape}")

        elif self.args.fed_alg in ['fed_opt','FAED']:
    ...
    def get_model_setting(self):
        ...
        if self.args.fed_alg == 'FAED': model_setting_str += '_gl' + str(self.args.global_learning_rate).replace('.','')
    ...
    def inference(self, dataloader):
        ...
        # 添加：将真实标签和预测标签保存到实例变量中
        self.test_true = all_true
    ...
    def average_weights(self):
        ...
        if self.args.fed_alg in ['fed_opt','FAED']:
        ...
        # # ABC integration only for FAED algorithm
        # if self.args.fed_alg == 'FAED':
        #     # Update teacher pool with client models and samples
        #     client_models = [{'model': update} for update in self.model_updates]  # Convert to expected format
        #     client_samples = self.num_samples_list
        #     self.abc.update_teacher_pool(client_models, client_samples)
        #
        #     # Get ABC aggregated weights
        #     abc_weights = self.abc.aggregate_knowledge()
        #
        #     # Blend original average with ABC weights (50-50 blend)
        #     for key in w_avg.keys():
        #         if key in abc_weights and w_avg[key].shape == abc_weights[key].shape:
        #             w_avg[key] = 0.5 * w_avg[key] + 0.5 * abc_weights[key]
