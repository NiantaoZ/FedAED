...
class BalancedAdaptiveModalityAlignment(nn.Module):
    ...
    def __init__(self, num_modalities=2, temperature=0.1):
        ...
        # 模态重要性权重 - 使用更稳定的初始化
        self.modal_importance = nn.Parameter(torch.ones(num_modalities, dtype=torch.float32) * 0.8)
        
        # 注册缓冲区用于稳定性
        self.register_buffer('running_quality_scores', torch.zeros(num_modalities))
        self.register_buffer('running_count', torch.zeros(num_modalities))
    ...
    def forward(self, modality_features, modality_mask):
        ...
        for i in range(self.num_modalities):
            if i < len(modality_features) and modality_features[i] is not None:
                ...
                var_score = torch.var(feat, dim=1, keepdim=True) + 1e-8
                ...
                # 平衡的质量分数计算
                quality = norm_score * 0.5 + var_score * 0.5
                quality_scores.append(quality)
                
                # 更新运行统计
                if self.training:
                    self.running_quality_scores[i] = 0.9 * self.running_quality_scores[i] + 0.1 * quality.mean().item()
                    self.running_count[i] += 1
            else:
        ...
        # 结合模态重要性和质量分数 - 更稳定的组合
        combined_scores = quality_scores * torch.sigmoid(self.modal_importance).unsqueeze(0)
        availability_dist = F.softmax(combined_scores / (self.temperature + 1e-8), dim=1)
        availability_dist = availability_dist * modality_mask.float()
        
        # 安全的归一化
        sum_weights = torch.sum(availability_dist, dim=1, keepdim=True) + 1e-8
        ...
        for i in range(self.num_modalities):
            if i < len(modality_features) and modality_features[i] is not None:
                ...
                for j in range(self.num_modalities):
                    if (i != j and modality_features[j] is not None and 
                        availability_dist[:, j].mean() > 0.2 and
                        self.running_quality_scores[j] > 0.3):
                        
                        # 更保守的权重分配
                        transfer_weight = availability_dist[:, j].unsqueeze(1) * 0.3
                        enhanced_feature = enhanced_feature * 0.7 + transfer_weight * modality_features[j].float() * 0.3
...
class BalancedCrossModalContrastive(nn.Module):
    """平衡的跨模态对比学习"""
    def __init__(self, margin=0.6, alpha=0.3, temperature=0.1):
    ...
    def safe_feature_alignment(self, feat1, feat2):
        ...
        # 使用更稳定的相似度计算
        cosine_sim = F.cosine_similarity(feat1_norm, feat2_norm, dim=1).clamp(-1.0, 1.0)
        mse_loss = F.mse_loss(feat1_norm, feat2_norm, reduction='mean')
        
        return (1 - cosine_sim.mean()) * 0.6 + mse_loss * 0.4
    ...
    def intra_modal_alignment(self, student_features, teacher_features):
        ...
        for s_feat, t_feat in zip(student_features, teacher_features):
            if s_feat is not None and t_feat is not None and s_feat.shape[0] > 0 and t_feat.shape[0] > 0:
                try:
    ...
    def inter_modal_alignment(self, features, labels):
        """模态间对齐 - 更稳定的实现"""
        valid_features = [feat for feat in features if feat is not None and feat.shape[0] > 0]
        ...
        for i in range(len(aligned_features)):
            for j in range(i + 1, len(aligned_features)):
                ...
                # 更稳定的对比损失计算
                positive_mask = label_sim > 0.5
                negative_mask = label_sim < 0.5
                
                if positive_mask.any():
                    pos_loss = torch.where(positive_mask, self.margin - similarity, torch.zeros_like(similarity))
                    pos_loss = pos_loss[positive_mask].mean()
                else:
                    pos_loss = torch.tensor(0.0, device=similarity.device)
                
                if negative_mask.any():
                    neg_loss = torch.where(negative_mask, similarity - (1 - self.margin), torch.zeros_like(similarity))
                    neg_loss = neg_loss[negative_mask].mean()
                else:
    ...
    def forward(self, student_features, teacher_features, labels):
        ...
        # 更平衡的权重分配
        return intra_loss * 0.7 + inter_loss * 0.3
...
class BalancedKnowledgeDistillation(nn.Module):
    ...
    def forward(self, student_logits, teacher_logits):
        ...
        if student_logits.shape != teacher_logits.shape:
            min_size = min(student_logits.shape[1], teacher_logits.shape[1])
        ...
        # 添加logits级别的MSE损失
        mse_loss = self.mse_loss(student_logits, teacher_logits.detach())
        
        # 平衡组合
        return kl_loss * (self.temperature ** 2) * 0.7 + mse_loss * 0.3
...
class ClientFAED(ClientFedAvg):
    def __init__(
        ...
        self.ccr_module = BalancedCrossModalContrastive(
            ...
            temperature=getattr(args, 'ccr_temperature', 0.1)
        ).to(device)
        ...
        # 平衡的损失权重
        self.lambda_ccr = getattr(args, 'lambda_ccr', 0.15)
        ...
        self.ema_alpha = 0.999  # 更保守的EMA系数
    ...
    def extract_balanced_features(self, x_a, x_b):
        ...
        # 模态A
        if x_a is not None and x_a.shape[0] > 0:
            x_a = x_a.float()
            if len(x_a.shape) > 2:
                # 多种特征提取方式的平衡
                feat_a = x_a.mean(dim=1)
                if feat_a.shape[1] < 8:
                    feat_a = x_a.view(x_a.size(0), -1)
                # 添加标准化
                feat_a = F.normalize(feat_a, dim=1)
            else:
                feat_a = F.normalize(x_a, dim=1)
    ...
    def get_dynamic_weights(self):
        ...
        if self.training_stage == 0:
            ...
            return self.lambda_ccr * 1.1, self.lambda_ama * 1.1, self.lambda_kd * 1.05
