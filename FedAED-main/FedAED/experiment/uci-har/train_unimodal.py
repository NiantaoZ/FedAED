...
from model.unimodal_models import ConvRNNClassifier
...
def parse_args():
    ...
    # 获取当前脚本的绝对路径
    base_path = Path(os.path.realpath(__file__)).parents[2]

    if path_conf["data_dir"] == ".":
        path_conf["data_dir"] = str(base_path.joinpath('data').resolve()).replace("\\", "/")
    if path_conf["output_dir"] == ".":
        path_conf["output_dir"] = str(base_path.joinpath('output').resolve()).replace("\\", "/")
